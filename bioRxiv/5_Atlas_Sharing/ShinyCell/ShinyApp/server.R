...
shinyServer(function(input, output, session) { 
  ...
  # Cargar los objetos con barra de progreso
  withProgress(message = "Loading CAR-T Atlas data...", {
    incProgress(0.25, detail = "Reading sc1conf")
    sc1conf <<- readRDS("sc1conf.rds")
    
    incProgress(0.5, detail = "Reading sc1def")
    sc1def  <<- readRDS("sc1def.rds")
    
    incProgress(0.75, detail = "Reading sc1gene")
    sc1gene <<- readRDS("sc1gene.rds")
    
    incProgress(1, detail = "Reading sc1meta")
 ...
 optCrt="{ option_create: function(data,escape) {return('<div class=\"create\"><strong>' + '</strong></div>');} }" 
  ...
  output$sc1a1.dt <- renderDataTable({
    withProgress(message = "Loading table...", {
      ...
      incProgress(1, detail = "Rendering table")
  ...
  output$sc1a1oup2 <- renderPlot({
    withProgress(message = "Rendering gene plot...", {
      incProgress(0.5, detail = "Preparing data")
  ...
  output$sc1b2oup1 <- renderPlot({
    withProgress(message = "Rendering co-expression plot...", {
      incProgress(0.5, detail = "Preparing data")
  ...
  output$sc1b2oup2 <- renderPlot({
    withProgress(message = "Rendering co-expression legend...", {
      incProgress(0.5, detail = "Preparing data")
      
      p <- scDRcoexLeg(
        input$sc1b2inp1, input$sc1b2inp2, 
        input$sc1b2col1, input$sc1b2fsz
      )
  ...
  output$sc1b2.dt <- renderDataTable({
    withProgress(message = "Loading co-expression table...", {
      incProgress(0.5, detail = "Processing data")
  ...
  output$sc1c1oup <- renderPlot({
    withProgress(message = "Rendering violin/box plot...", {
      incProgress(0.5, detail = "Preparing data")
...
### Plots for tab c2 
  ...
  output$sc1c2oup <- renderPlot({
    withProgress(message = "Rendering proportion plot...", {
      incProgress(0.5, detail = "Preparing data")
...
output$sc1c2oup.png <- downloadHandler( 
  ...
  output$sc1d1oup <- renderPlot({
    withProgress(message = "Rendering bubble heatmap...", {
      incProgress(0.5, detail = "Preparing data")
