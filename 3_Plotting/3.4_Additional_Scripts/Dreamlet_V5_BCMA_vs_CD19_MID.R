...
# Program: Dreamlet_V5_BCMA_vs_CD19_MID.R
...
counts_df$Time_Point_Ranges <- factor(counts_df$Time_Point_Ranges,
  levels = c(
    "Patient_Infusion_Product", "<2_weeks",
...
counts_df_2 <- unique_df %>%
  group_by(Norm_Patient_Name, Product_norm, Time_Point_Ranges, ScFv, Costim_Domain_1) %>%
  summarise(.groups = "drop")

dim(counts_df_2)

counts_df_2 <- counts_df_2[counts_df_2$Time_Point_Ranges %in% c("Patient_Infusion_Product", "2_weeks-3_months"), ]
dim(counts_df_2)
print(counts_df_2, n = 100)

counts_df_2 <- counts_df_2[counts_df_2$Time_Point_Ranges %in% c("2_weeks-3_months"), ]
dim(counts_df_2)
print(counts_df_2, n = 100)
...
setwd("/home/scamara/data_a/scamara/Atlas/Resultados_V5/BCMA_vs_CD19_MID")
if(TRUE){
  write.csv(Final_result, "Final_result_Fig_2J_MID.csv", row.names = FALSE)
} else{
  Final_result <- read.csv("Final_result_Fig_2J_MID.csv")
}
