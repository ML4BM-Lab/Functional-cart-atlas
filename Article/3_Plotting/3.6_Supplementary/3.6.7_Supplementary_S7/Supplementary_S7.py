...
# %% S7Ba - Filter object
filtered_adata = adata_orig[adata_orig.obs["Antigen"] == "Blood"].copy()
...
# %% S7Ba - Remove IACs (Monocyte-like T cells)
noIACs_adata = filtered_adata[
    filtered_adata.obs["manual_celltype_annotation_high"] != "Monocyte-like T cells"
].copy()

print(noIACs_adata.shape[0])

# %% S7Ba - Plot UMAP
fig, ax = plt.subplots(figsize=(8, 8))
ax.scatter(
    noIACs_adata.obsm["X_umap"][:, 0],
    noIACs_adata.obsm["X_umap"][:, 1],
    c="lightgrey",
    s=10,
    alpha=0.8,
    linewidths=0
...
ax.axis("off")
plt.tight_layout()
plt.savefig(_output_path(figures_dir, "Figura_S7Ba.pdf"))
plt.show()

# %% S7Bb - Artificial doublets vs IACs
adataB = adata_orig.copy()

## Normalize and log data
# 1. Normalize total counts per cell
sc.pp.normalize_total(adataB, target_sum=1e4)
...
print("Matrix used for doublets: (cells x genes):", X.shape)
...
sc.pl.umap(
    ...
    title=None,
    save="_S7Bb_IACs_vs_Doublets.pdf"
)
...
## Save it
ggsave(plot, _output_path(figures_dir, "S7C_IACs_Cluster_Proportion.pdf"))
