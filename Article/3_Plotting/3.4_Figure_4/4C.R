...
filtered_sce <- filtered_sce[, colData(filtered_sce)$STATUS == "DISEASE"] # Not needed
print((filtered_sce %>% dim())[2]) # Not needed

filtered_sce <- filtered_sce[, !(colData(filtered_sce)$Stimulated == "YES" & colData(filtered_sce)$Stimulation_Location == "In_vitro")] # Not needed
print((filtered_sce %>% dim())[2]) # Not needed
...
.current_dir <- file.path(project_dir, "Figuras", "Figura_4")
pdf(.output_path(.current_dir, "4C.pdf"))
plotVolcano(res.compare, coef = "compare", ncol = 4, nGenes=10)
dev.off()
