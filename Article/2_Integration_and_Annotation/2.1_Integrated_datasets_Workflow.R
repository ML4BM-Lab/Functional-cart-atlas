...
# Program: 2.1_Integrated_datasets_Workflow.R
...
.generated_input_paths <- c(
    ...
    file.path(project_dir, "Resultados", "Deng_et_al", "RDS", "Normalized_CellRanger_Deng_RDS.rds"),
    file.path(project_dir, "Resultados", "Good_et_al", "RDS", "Normalized_CellRanger_Good_RDS.rds"),
    file.path(project_dir, "Resultados", "Lynn_et_al", "RDS", "Normalized_CellRanger_Lynn_RDS.rds"),
    file.path(project_dir, "Resultados", "Melenhorst_et_al", "RDS", "Normalized_CellRanger_Melenhorst_RDS.rds"),
    file.path(project_dir, "Resultados", "Sheih_et_al", "RDS", "Normalized_CellRanger_Sheih_RDS.rds"),
    ...
    file.path(project_dir, "Resultados", "Li_X_et_al", "RDS", "Normalized_CellRanger_Li_RDS.rds"),
...
# V4 - Remove the custom CAR genes #
counts <- NULL
Seurat_list_Bai_V4 <- list()
for (i in seq_along(Seurat_list_Bai)){
    counts <- GetAssayData(Seurat_list_Bai[[i]], assay = "RNA")
    counts <- counts[-(which(rownames(counts) == "CAR")),]
    Seurat_list_Bai_V4[[i]] <- subset(Seurat_list_Bai[[i]], features = rownames(counts))
    counts <- NULL

    names(Seurat_list_Bai_V4)[i] <- names(Seurat_list_Bai)[i]

    print(paste0("DONE for ", names(Seurat_list_Bai_V4)[i]))
}

rm(Seurat_list_Bai)
Seurat_list_Bai_V4[[1]]@meta.data %>% head()
...
# V4 - Remove the custom CAR genes #
counts <- NULL
Seurat_list_Deng_V4 <- list()
for (i in seq_along(Seurat_list_Deng)){
    counts <- GetAssayData(Seurat_list_Deng[[i]], assay = "RNA")
    counts <- counts[-(which(rownames(counts) == "FMC63-28Z")),]
    Seurat_list_Deng_V4[[i]] <- subset(Seurat_list_Deng[[i]], features = rownames(counts))
    counts <- NULL

    names(Seurat_list_Deng_V4)[i] <- names(Seurat_list_Deng)[i]

    print(paste0("DONE for ", names(Seurat_list_Deng_V4)[i]))
}
...
# V4 - Remove the custom CAR genes #
counts <- NULL
Seurat_list_Good_V4 <- list()
for (i in seq_along(Seurat_list_Good)){
    counts <- GetAssayData(Seurat_list_Good[[i]], assay = "RNA")
    counts <- counts[-(which(rownames(counts) %in% c("axicel", "CAR19-28Z-Mackall"))),]
    Seurat_list_Good_V4[[i]] <- subset(Seurat_list_Good[[i]], features = rownames(counts))
    counts <- NULL

    names(Seurat_list_Good_V4)[i] <- names(Seurat_list_Good)[i]

    print(paste0("DONE for ", names(Seurat_list_Good_V4)[i]))
}
...
# V4 - Remove the custom CAR genes #
counts <- NULL
Seurat_list_Melenhorst_V4 <- list()
for (i in seq_along(Seurat_list_Melenhorst)){
    counts <- GetAssayData(Seurat_list_Melenhorst[[i]], assay = "RNA")
    counts <- counts[-(which(rownames(counts) == "muCD19BBz")),]
    Seurat_list_Melenhorst_V4[[i]] <- subset(Seurat_list_Melenhorst[[i]], features = rownames(counts))
    counts <- NULL

    names(Seurat_list_Melenhorst_V4)[i] <- names(Seurat_list_Melenhorst)[i]

    print(paste0("DONE for ", names(Seurat_list_Melenhorst_V4)[i]))
}
...
# V4 - Remove the custom CAR genes #
counts <- NULL
Seurat_list_Wang_V4 <- list()
for (i in seq_along(Seurat_list_Wang)){
    counts <- GetAssayData(Seurat_list_Wang[[i]], assay = "RNA")
    counts <- counts[-(which(rownames(counts) == "IFNB1-5LTR")),]
    Seurat_list_Wang_V4[[i]] <- subset(Seurat_list_Wang[[i]], features = rownames(counts))
    counts <- NULL

    names(Seurat_list_Wang_V4)[i] <- names(Seurat_list_Wang)[i]

    print(paste0("DONE for ", names(Seurat_list_Wang_V4)[i]))
}
...
# V4 - Remove the custom CAR genes #
counts <- NULL
Seurat_list_Haradvala_V4 <- list()
for (i in seq_along(Seurat_list_Haradvala)){
    counts <- GetAssayData(Seurat_list_Haradvala[[i]], assay = "RNA")
    counts <- counts[-(which(rownames(counts) %in% c("Yescarta", "Kymriah"))),]
    Seurat_list_Haradvala_V4[[i]] <- subset(Seurat_list_Haradvala[[i]], features = rownames(counts))
    counts <- NULL

    names(Seurat_list_Haradvala_V4)[i] <- names(Seurat_list_Haradvala)[i]

    print(paste0("DONE for ", names(Seurat_list_Haradvala_V4)[i]))
}
...
# V4 - Remove the custom CAR genes #
counts <- NULL
Seurat_list_Li_X_letter_V4 <- list()
for (i in seq_along(Seurat_list_Li_X_letter)){
    counts <- GetAssayData(Seurat_list_Li_X_letter[[i]], assay = "RNA")
    counts <- counts[-(which(rownames(counts) %in% c("Yescarta", "Kymriah"))),]
    Seurat_list_Li_X_letter_V4[[i]] <- subset(Seurat_list_Li_X_letter[[i]], features = rownames(counts))
    counts <- NULL

    names(Seurat_list_Li_X_letter_V4)[i] <- names(Seurat_list_Li_X_letter)[i]

    print(paste0("DONE for ", names(Seurat_list_Li_X_letter_V4)[i]))
}
...
Bai_merged <- merge(
    x = Seurat_list_Bai_V4[[1]],
    y = c(Seurat_list_Bai_V4[[2]], Seurat_list_Bai_V4[[3]], Seurat_list_Bai_V4[[4]], Seurat_list_Bai_V4[[5]], Seurat_list_Bai_V4[[6]], Seurat_list_Bai_V4[[7]], Seurat_list_Bai_V4[[8]], Seurat_list_Bai_V4[[9]])
)

Seurat_merged <- merge(x = Bai_merged, y = c(
    Seurat_list_Boroughs, Seurat_list_Deng_V4, Seurat_list_Good_V4, Seurat_list_Lynn, Seurat_list_Melenhorst_V4,
    Seurat_list_Sheih, Seurat_list_Wang_V4, Seurat_list_Xhangolli, Seurat_list_Li_X, Seurat_list_Rodriguez_Marquez,
    Seurat_list_Haradvala_V4, Seurat_list_Li_X_letter_V4
))

Seurat_merged # 444142 cells

## Identification of NON-CD3+ population inside Good's dataset!
if (Identificacion_Good_CD3) {
    pdf(.output_path(project_dir, "Resultados", "Joined_datasets", "Integration", "Plots", "WO_integ", "motivos_para_excluir_parte_GOOD.pdf"))
    FeaturePlot(object = Seurat_merged, features = c("CD3D", "CD4", "CD8A"), raster = FALSE, order = TRUE)
    dev.off()
}

## Remove NON-CD3+ population in all datasets
Seurat_merged <- subset(x = Seurat_merged, subset = CD3D > 0)
Seurat_merged # 414780 cells
...
colors_palette <- c("#CDFD02", "#6C35FF", "#FBA3B4", "#CBC7FE", "#2B097E", "#FE4F11", "#8E0D0D", "#A9FCFA", "#D35E82", "#CECECE", "#FFC49C", "#383838", "#00A57E")

# Plot UMAP
pdf(.output_path(project_dir, "Resultados", "Joined_datasets", "Integration", "Plots", "V4", "WO_integ", "WO_integ_Seurat.pdf"))
...
pdf(.output_path(project_dir, "Resultados", "Joined_datasets", "Integration", "Plots", "V4", "WO_integ", "QC_metrics_WO_integ_Seurat.pdf"))
...
# Cell cycle plot
pdf(.output_path(project_dir, "Resultados", "Joined_datasets", "Integration", "Plots", "V4", "WO_integ", "WO_integ_Seurat_cellcycle_scoring.pdf"))
DimPlot(Seurat_merged,
    group.by = "Phase",
    reduction = "umap_wo_integ",
    raster = FALSE,
...
saveRDS(Seurat_merged, .output_path(project_dir, "Resultados", "Joined_datasets", "Integration", "RDS", "WO_integ", "V4", "Seurat_merged_WO_integ.RDS"))

## Add automatically cell type information using CellTypist
.current_dir <- file.path(project_dir, "Resultados", "Joined_datasets", "Integration", "Python-Celltypist", "V4")
## Create or load the h5ad file if necessary
if (Create_h5ad) {
...
############## PAUSE ##############
# Here, you should switch to Python ("2.2_Celltypist_annotation.py") to obtain the CellTypist results
###################################
...
Seurat_merged <- readRDS(.input_path(project_dir, "Resultados", "Joined_datasets", "Integration", "RDS", "WO_integ", "V4", "Seurat_merged_WO_integ.RDS"))
Seurat_merged <- AddMetaData(Seurat_merged, metadata = celltypist_metadata)

## Celltypist annotations plot
pdf(.output_path(project_dir, "Resultados", "Joined_datasets", "Integration", "Plots", "V4", "WO_integ", "Celltypist_ann_WO_integ_Seurat.pdf"))
