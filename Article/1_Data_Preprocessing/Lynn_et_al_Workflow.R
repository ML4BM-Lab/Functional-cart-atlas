...
for (contador in Patient_list_CD19_vs_GD2) {
    ...
    Seurat_list_Lynn_CD19_vs_GD2[[contador]] <- RenameCells(Seurat_list_Lynn_CD19_vs_GD2[[contador]], new.names = gsub("-1", "", colnames(Seurat_list_Lynn_CD19_vs_GD2[[contador]])))
    print(Seurat_list_Lynn_CD19_vs_GD2[[contador]])
...
for (contador in Patient_list_Control_vs_JUN) {
    ...
    Seurat_list_Lynn_Control_vs_JUN[[contador]] <- RenameCells(Seurat_list_Lynn_Control_vs_JUN[[contador]], new.names = gsub("-1", "", colnames(Seurat_list_Lynn_Control_vs_JUN[[contador]])))
    print(Seurat_list_Lynn_Control_vs_JUN[[contador]])
    print(paste0(contador, " DONE"))
}

rm(contador, cell_ranger_dir_Control_vs_JUN, cell_ranger_data_Control_vs_JUN, Patient_list_Control_vs_JUN)

Seurat_list_Lynn <- c(Seurat_list_Lynn_CD19_vs_GD2, Seurat_list_Lynn_Control_vs_JUN)
Patient_list <- names(Seurat_list_Lynn)

rm(KSeurat_list_Lynn_CD19_vs_GD2, Seurat_list_Lynn_Control_vs_JUN)


##### Quality control #####

### Add more metadata: novelty score and mito ratio
for (contador in seq_along(Seurat_list_Lynn)) {
    Seurat_list_Lynn[[contador]]$log10GenesPerUMI <- log10(Seurat_list_Lynn[[contador]]$nFeature_RNA) / log10(Seurat_list_Lynn[[contador]]$nCount_RNA)

    Seurat_list_Lynn[[contador]]$mitoRatio <- (PercentageFeatureSet(object = Seurat_list_Lynn[[contador]], pattern = "^MT-")) / 100
...
if (exists("Pre_filter")) {
    ...
    for (contador in seq_along(Seurat_list_Lynn)) {
        if (min(Seurat_list_Lynn[[contador]]$nCount_RNA) >= minCov) {
            countLOW[[contador]] <- min(Seurat_list_Lynn[[contador]]$nCount_RNA)
        } else {
            countLOW[[contador]] <- quantile(Seurat_list_Lynn[[contador]]$nCount_RNA, prob = 0.01)
        }
        countHIGH[[contador]] <- quantile(Seurat_list_Lynn[[contador]]$nCount_RNA, prob = 0.99)
        featureLOW[[contador]] <- quantile(Seurat_list_Lynn[[contador]]$nFeature_RNA, prob = 0.01)
        featureHIGH[[contador]] <- quantile(Seurat_list_Lynn[[contador]]$nFeature_RNA, prob = 0.99)

        ## subset
        Seurat_list_Lynn[[contador]] <- subset(Seurat_list_Lynn[[contador]], subset = (nFeature_RNA > featureLOW[[contador]]) &
...
for (cont in seq_along(Seurat_list_Lynn)) {
    merged_metadata_df[[cont]] <- Seurat_list_Lynn[[cont]]@meta.data
}
rm(cont)
...
# Values
# Order: Exp1_CD19, Exp1_GD2, Exp2_Control, Exp2_JUN

Min_feat <- c(1600, 1800, 900, 900)
Min_counts <- c(5000, 5000, 2000, 1750)
Max_counts <- c(26000, 32000, 18000, 24000)
Max_mito_ratio <- c(0.09, 0.09, 0.09, 0.09)
...
for (i in Patient_list) {
    ...
    bcrank <- DropletUtils::barcodeRanks(counts(sce_Seurat_list_Lynn[[i]]))
...
merged_metadata_df %>%
    ...
    geom_vline(xintercept = Min_counts[4], col = colores[4], alpha = 0.6, linetype = "dotted") +
    geom_vline(xintercept = Max_counts[4], col = colores[4], alpha = 0.6, linetype = "dotted") +
...
merged_metadata_df %>%
    ...
    geom_vline(xintercept = Min_feat[4], col = colores[4], alpha = 0.6, linetype = "dotted") +
...
for (i in seq_along(Seurat_list_Lynn)) {
    p1[[i]] <- ggplot(data = Seurat_list_Lynn[[i]]@meta.data, aes(x = nCount_RNA, y = nFeature_RNA, color = mitoRatio)) +
        geom_point(size = 0.7) +
...
p1 <- list()
for (i in seq_along(Seurat_list_Lynn)) {
    p1[[i]] <- ggplot(data = Seurat_list_Lynn[[i]]@meta.data, aes(x = nCount_RNA, y = mitoRatio)) +
        geom_point(size = 0.7) +
        theme_classic() +
        ggtitle(paste0(Patient_list[i]))
}
...
for (i in seq_along(Seurat_list_Lynn)) {
    Seurat_list_Lynn[[i]] <- subset(Seurat_list_Lynn[[i]],
...
Seurat_doublets_finder <- Seurat_list_Lynn

sweep_res_list_Lynn <- list()
sweep_stats_list_Lynn <- list()
bcmvn_list_Lynn <- list()
pK <- list()
BCmetric <- list()
pK_choose <- list()
nExp <- list()
per_doubl <- c(0.008, 0.008, 0.056, 0.09) # Calculated looking at 10X table
...
for (i in seq_along(Seurat_doublets_finder)) {
    ...
    sweep_res_list_Lynn[[i]] <- paramSweep_v3(Seurat_doublets_finder[[i]], PCs = 1:10, sct = FALSE, num.cores = 12)
    sweep_stats_list_Lynn[[i]] <- summarizeSweep(sweep_res_list_Lynn[[i]], GT = FALSE)
    bcmvn_list_Lynn[[i]] <- find.pK(sweep_stats_list_Lynn[[i]])
    dev.off()

    pK[[i]] <- as.numeric(as.character(bcmvn_list_Lynn[[i]]$pK))
    BCmetric[[i]] <- bcmvn_list_Lynn[[i]]$BCmetric
...
for (i in seq_along(Seurat_doublets_finder)) {
    ...
    Seurat_list_Lynn[[i]] <- subset(Seurat_list_Lynn[[i]], cells = Singlets)
    print(Seurat_list_Lynn[[i]])
...
p1 <- list()
for (i in seq_along(Seurat_list_Lynn)) {
    p1[[i]] <- ggplot(data = Seurat_list_Lynn[[i]]@meta.data, aes(x = nCount_RNA, y = nFeature_RNA, color = mitoRatio, grou)) +
        geom_point(size = 0.7) +
        scale_color_gradientn(colors = c("black", "blue", "green2", "red", "yellow")) +
        stat_smooth(method = lm) +
        scale_x_log10() +
...
for (i in seq_along(Seurat_list_Lynn)) {
    ...
    Seurat_list_Lynn[[i]] <- CellCycleScoring(Seurat_list_Lynn[[i]],
        g2m.features = g2m_genes,
        s.features = s_genes)

    Seurat_list_Lynn[[i]] <- FindVariableFeatures(Seurat_list_Lynn[[i]],
        selection.method = "vst",
        nfeatures = 2000)

    Seurat_list_Lynn[[i]] <- ScaleData(Seurat_list_Lynn[[i]])
...
Seurat_list_Lynn <- Seurat_list_Lynn_2
rm(Seurat_list_Lynn_2)

Seurat_list_Lynn <- FindVariableFeatures(Seurat_list_Lynn,
    selection.method = "vst",
    nfeatures = 2000
)

Seurat_list_Lynn <- ScaleData(Seurat_list_Lynn)

# Run PCA
Seurat_list_Lynn <- RunPCA(object = Seurat_list_Lynn, reduction.name = "pca_wo_integ")

# ElbowPlot(Seurat_list_Lynn, ndims = 50, reduction = "pca_wo_integ")
# dev.off()

# Run UMAP
Seurat_list_Lynn <- RunUMAP(Seurat_list_Lynn,
    dims = 1:25,
...
p <- FeaturePlot(Seurat_list_Lynn,
    reduction = "umap_wo_integ",
